...
**Logging has no C++ counterpart.** `esphome/components/ct002/balancer.cpp` carries no log statements at all — the firmware's diagnostics are ESPHome's own — so a Python-side log line (e.g. `LoadBalancer._log_steer`, the per-consumer DEBUG record of how each command was decided) is added on the Python side only and does not break parity. The rule covers behaviour; a log statement changes none.
