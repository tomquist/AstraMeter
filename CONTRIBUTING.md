...
**`esphome/components/tibber_pulse/`** is the one powermeter with a firmware counterpart, a sensor platform that feeds `ct002:` like any other sensor. It mirrors two pieces of the Python source. The first is the endpoint fallback in `TibberPulse._fetch_telegram` (`powermeter/tibber_pulse.py`), mirrored by `bridge_endpoint.{h,cpp}`. The second is `parse_sml_powers` / `EnergyStats.from_sml_frame` (`powermeter/sml.py`), mirrored by `sml_power.{h,cpp}`. Both are std-only, so host gtests drive them. `tests/components/tibber_pulse/_gen_sml_vectors.py` runs the *Python* decoder over a set of telegrams and writes what it returns into `host_sml_vectors.h`, which the gtest holds the C++ decoder to. Add a telegram there when decoding changes, and regenerate; a pytest fails while the committed header is stale. Two things are deliberately not mirrored. The WebSocket push stream is left out, because stock ESPHome has no WebSocket client. A telegram without power registers is "no reading" on the ESP, not Python's single 0 W. `test_tibber_pulse_host_e2e.py` runs the compiled host binary against a fake bridge.
