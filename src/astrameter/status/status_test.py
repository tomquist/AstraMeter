...
def test_materialize_copies_the_running_config(tmp_path):
    source = tmp_path / "running.ini"
    source.write_text("[GENERAL]\nDEVICE_TYPE = ct002\n")
    target_dir = tmp_path / "config"
    written = materialize_config(str(source), "astrameter.ini", str(target_dir))
    with open(written, encoding="utf-8") as handle:
        body = handle.read()
    assert "DEVICE_TYPE = ct002" in body
    assert "Generated by AstraMeter" in body
...
def test_materialize_never_clobbers_an_existing_file(tmp_path):
    ...
    source = tmp_path / "running.ini"
    source.write_text("[GENERAL]\n")
    materialize_config(str(source), "astrameter.ini", str(target_dir))
