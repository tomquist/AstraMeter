//! `sma_energy_meter` — port of `src/astrameter/powermeter/sma_energy_meter.py`. Real implementation
//! lands in Phase 2/3.
...
pub fn create(_section: &Section<'_>, _platform: Arc<Platform>) -> Result<Arc<dyn Powermeter>> {
    Err(Error::config(
        "sma_energy_meter powermeter not yet implemented in Rust port",
    ))
}
