...
- **Improved** active control to stop chasing power-meter noise: when a battery is hunting (its correction keeps reversing sign because it's reacting to jitter or a fluctuation smaller than its own deadband), tiny corrections are now dropped instead of issued every poll. In realistic households this cut wasted battery movement/wear by ~17 % and actually *improved* self-consumption — chasing the noise was itself pushing the grid back and forth — while genuine load/solar steps still react immediately. On by default; tune or disable via `HUNT_DEADBAND_EXTRA` (`hunt_deadband_extra` on ESPHome) ([#514](https://github.com/tomquist/astrameter/pull/514)).
