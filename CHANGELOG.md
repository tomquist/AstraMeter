...
- **Added** the same **live status dashboard to the ESPHome component**, served by the ESP32 itself at `http://<device>/`. Adding `dashboard:` under `ct002:` is the whole opt-in, and `dashboard: false` takes it back out of the firmware. Grid power, every battery with its target and saturation, the sensor's health, the balancer's internals and its control-quality verdict are all there; add `controls: true` to steer batteries from the page as well (off by default — the page has no login of its own). Configuration is the one thing it cannot show, because on an ESP32 that lives in the firmware. See [docs/dashboard.md](docs/dashboard.md#esphome-on-an-esp32) ([#605](https://github.com/tomquist/astrameter/pull/605)).
