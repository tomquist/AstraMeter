...
- **Fixed** the Shelly emulator (`shellypro3em`/`shellyemg3`/`shellyproem50`) going permanently unresponsive after a transient UDP socket error, which left the Marstek battery unable to reach the meter until a restart. The emulator now ignores non-fatal socket errors and automatically rebinds its listener on connection loss, matching the CT002/CT003 emulator's existing recovery behavior ([#404](https://github.com/tomquist/astrameter/issues/404)).
