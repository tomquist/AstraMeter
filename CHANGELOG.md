...
- **Added** opt-in **HTTP cloud reporting** for the CT002/CT003 emulator (Python and ESPHome): when enabled, AstraMeter mimics a real CT by periodically sending the same `setCtReporting` request to Marstek's cloud (`hamedata.com`) with live grid power, the per-phase charge/discharge split, Wi-Fi signal, battery count and link state. Off by default; the reported device id is the CT's MAC (the device registered via your Marstek account when one is configured), and the model and firmware version are sent automatically, so you typically only set the interval. See [docs/marstek-mqtt-http.md](docs/marstek-mqtt-http.md#6-http-cloud-reporting-both-models) ([#500](https://github.com/tomquist/astrameter/pull/500)).
