...
- **Improved** active control so it holds the grid much closer to zero: it pulls noticeably less power from the grid and spills less back to it, reacts faster after load and solar changes, and greatly reduces the overshoot and back-and-forth hunting some setups saw — all without you tuning anything for your particular power meter. It adapts on its own, including to meters that report with a delay (e.g. a Home Assistant push sensor), which previously made the system over-correct and oscillate. In simulation across a wide range of realistic households it cuts the energy needlessly imported from and exported to the grid by ~55–60%, worst-case overshoot by ~65% and the grid's peak-to-peak swing by ~50%, and battery wear by ~70%, while settling ~35% faster. On by default; turn it down or off with the new **Grid Prediction** option in the Home Assistant add-on configuration (`GRID_PREDICT_TRUST` in a config file, `grid_predict_trust` on ESPHome).
