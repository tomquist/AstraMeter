...
- **Added** opt-in **HTTP cloud reporting** for the CT002/CT003 emulator: when enabled (`CLOUD_REPORTING = true`), AstraMeter mimics a real CT by periodically sending the same `setCtReporting` request to Marstek's cloud (`hamedata.com`) with live grid power, the per-phase charge/discharge split, Wi-Fi signal, battery count and link state. Off by default; configure the device id, account id and interval to match a paired device. See [docs/marstek-mqtt-http.md](docs/marstek-mqtt-http.md#6-http-cloud-reporting-both-models) ([#500](https://github.com/tomquist/astrameter/pull/500)).
