...
- **Added** an opt-in **live status dashboard**: one page showing grid power, every battery's target and reported power, power-source health and the balancer's internal state, plus the ability to change your configuration and steer individual batteries without editing files. Always available in the Home Assistant add-on, straight from the sidebar; for Docker, set `DASHBOARD_ENABLED`. See [docs/dashboard.md](docs/dashboard.md) ([#577](https://github.com/tomquist/astrameter/pull/577)).
