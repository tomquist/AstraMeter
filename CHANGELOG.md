...
- **Fixed** continuous grid oscillation under a slow/laggy power meter ([#473](https://github.com/tomquist/astrameter/issues/473)): when the meter reading the controller acts on lags reality (e.g. a Home Assistant push sensor with measurement delay), active control could hunt back and forth indefinitely — the grid never settling at zero — instead of tracking. Per-battery corrections are now **oscillation-damped**: the balancer detects when a battery's command keeps reversing direction (the signature of hunting, not of a real load change) and bleeds the loop gain that sustains it, while leaving same-direction reactions to genuine load/solar steps at full speed. In simulation this cuts the sustained grid swing and hunting rate by roughly a third to a half on the affected setups (and improves settling/overshoot on several multi-battery scenarios too) with no slowdown elsewhere; tune or disable with the new `OSC_DAMP_MAX` / `OSC_DAMP_ALPHA` / `OSC_DAMP_DECAY` options (`osc_damp_max` / `osc_damp_alpha` / `osc_damp_decay` on ESPHome).
- **Fixed** several issues with multi-battery control that made the system react slowly or hunt back and forth after a load or solar change — batteries now settle faster and with less overshoot, especially when you have batteries that poll at different speeds or run with efficiency optimization enabled ([#469](https://github.com/tomquist/astrameter/issues/469)).
