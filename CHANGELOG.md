...
- **Changed** the **Force Rotation** button on the CT002/CT003 device in Home Assistant to appear only when efficiency rotation is enabled (`min_efficient_power` > 0); with every battery kept active there is nothing to rotate, so the button is no longer surfaced.
