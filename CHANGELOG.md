...
- **Changed** the bundled **battery simulator** to steer like a real Marstek Venus-class battery by default: instead of the old `output + grid` integral rule, each simulated battery now runs the gain-scheduled, accelerating self-consumption controller (deadband, ramp/acceleration curve, and power-window clamp) the real hardware uses, so simulated dynamics match hardware much more closely. The previous behavior is still available with `steering = "integral"` per battery.
