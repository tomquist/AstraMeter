...
- **Added** ways to turn **Active Control** off without hand-editing a config file: an **Active Control** option in the Home Assistant add-on's Configuration tab (and the web config generator's add-on output), plus a live **Active Control** switch on the CT002/CT003 device in Home Assistant (Python and ESPHome). Both default on; turning it off falls back to relay mode (the equivalent of `ACTIVE_CONTROL = False`). The switch's choice is published retained so it survives an AstraMeter restart. (The switch replaces the previous read-only "Active Control" status sensor.)
- **Added** an adaptive grid predictor that holds the grid closer to zero — less import/export, overshoot and oscillation — by adapting automatically to your power meter, including meters that report with a delay. On by default; adjust or disable via the new **Grid Prediction** option in the Home Assistant add-on (`GRID_PREDICT_TRUST` in a config file, `grid_predict_trust` on ESPHome).
- **Added** **deadband concentration** for multi-battery setups, reducing avoidable grid import/export. When two or more batteries share a phase, a small grid error gets split between them — and each battery's slice can fall below its firmware's ~20 W input deadband, so none of them act and the grid is left sitting slightly off zero (a few watts of avoidable import/export per battery). The balancer now hands a small near-zero error to a single battery instead, so it clears the deadband. Across the steering-evaluation suite this lowers avoidable grid import/export by ~4% overall (up to ~18% in mixed-cadence / solar / B2500 scenarios) while also reducing oscillation and battery travel in aggregate. On by default; tune the threshold or disable it with `CONCENTRATE_DEADBAND` (watts; `0` disables) in a config file or `concentrate_deadband` on ESPHome.
...
- **Changed** active control to work *with* the battery firmware's own control loop instead of fighting it ([#458](https://github.com/tomquist/astrameter/issues/458)): per-battery commands are now **ramp-paced** — each poll's delta is capped starting at the firmware ramp's first step (~30 W) and growing toward 100 W only while the battery demonstrably follows — and the balance deadband default was raised from 15 W to 25 W (above the battery's own ±20 W input deadband). The pacing caps, oscillation damping and adaptive grid-predictor trust were jointly retuned against the steering-evaluation suite: in simulation across realistic household scenarios this cuts worst-case opposite-direction overshoot and battery power churn substantially (with the faster predictor keeping real load/solar reactions quick despite the tighter caps). Tune or disable with the new `PACE_BASE_STEP` / `PACE_MAX_STEP` options (`pace_base_step` / `pace_max_step` on ESPHome). A new evaluation harness (`python -m astrameter.simulator.evaluation`) measures reaction/oscillation/energy metrics per scenario, and CI posts a before/after comparison on every PR.
