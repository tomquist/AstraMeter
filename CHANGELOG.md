...
- **Fixed** the Home Assistant add-on saying nothing when `custom_config` names a file that isn't there — it quietly ignored the setting and started from the add-on options instead, so a typo looked like the add-on ignoring your config for no reason. The log now names the file it could not find ([#578](https://github.com/tomquist/astrameter/pull/578)).
