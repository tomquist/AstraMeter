...
- **Improved** active control so the grid holds closer to zero — less import/export, overshoot and hunting — and the batteries settle faster after a load or solar change with much less power churn, lowering both your electricity bill and battery wear. The controller now works *with* each battery's own firmware loop instead of fighting it: per-battery commands are ramp-paced and oscillation-damped, an adaptive grid predictor compensates for meters that report with a delay (e.g. a Home Assistant push sensor), a steady-import trim recovers the self-consumption the firmware's input deadband leaves importing at the retail tariff, multi-battery setups hand a small near-zero grid error to a single battery so it clears the firmware deadband, and multi-battery efficiency mode no longer thrashes a battery in and out of the pool on noisy load. All on by default; tune or disable via the new `GRID_PREDICT_TRUST`, `IMPORT_TRIM_W`, `CONCENTRATE_DEADBAND`, `PACE_BASE_STEP` / `PACE_MAX_STEP`, `OSC_DAMP_*` and `EFFICIENCY_DEMAND_ALPHA` options (lowercase equivalents on ESPHome). The balance deadband default was also raised from 15 W to 25 W ([#458](https://github.com/tomquist/astrameter/issues/458), [#469](https://github.com/tomquist/astrameter/issues/469), [#473](https://github.com/tomquist/astrameter/issues/473)).
