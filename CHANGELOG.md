...
- **Fixed** the CT002/CT003 **Poll Interval** sensor reporting how often AstraMeter *answered* a battery rather than how often that battery actually polled, which made a `DEDUPE_TIME_WINDOW` look like it had been rounded up to the next whole second. Poll Interval is now the battery's own rate, a new **Answer Interval** sensor shows how often it gets a reply, and a battery whose polls the dedupe window suppresses is no longer at risk of being dropped as if it had gone silent. The dashboard and the ESPHome component report the same pair ([#589](https://github.com/tomquist/astrameter/issues/589)).
...
- **Added** an opt-in **live status dashboard**: one page showing grid power, your batteries, power-source health and — when AstraMeter is steering them — each battery's target and the balancer's internal state, plus the ability to change your configuration and steer individual batteries without editing files. Always available in the Home Assistant add-on, straight from the sidebar; for Docker, set `DASHBOARD_ENABLED`. See [docs/dashboard.md](docs/dashboard.md) ([#577](https://github.com/tomquist/astrameter/pull/577)).
