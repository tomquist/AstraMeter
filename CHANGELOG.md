...
- **Added** an **Active Control** switch to the CT002/CT003 device in Home Assistant (Python and ESPHome), so active control can be turned off — falling back to relay mode, the live equivalent of `ACTIVE_CONTROL = False` — and back on without editing the config. It defaults on, and the choice is published retained so it survives an AstraMeter restart. (Replaces the previous read-only "Active Control" status sensor.)
