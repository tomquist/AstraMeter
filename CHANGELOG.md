...
- **Changed** the CT002/CT003 emulator's relay mode (`ACTIVE_CONTROL = False`) to match real CT hardware more faithfully: the per-phase cross-talk charge/discharge buckets now forward each battery's **reported** power instead of reported+grid ([#457](https://github.com/tomquist/astrameter/issues/457)); inspection-mode (`'0'`) reporters now aggregate into the `x_*` bucket and combined-mode (phase `D`) reporters into the `ABC_*` bucket / `ABC_chrg_nb` count, instead of both being lumped into phase A and skewing the relay share split ([#460](https://github.com/tomquist/astrameter/issues/460)); and a battery that goes silent now drops out of the counts/aggregates after missing ~2 of its own poll cycles (like the real CT) instead of after a fixed 120 s — set `CONSUMER_TTL` (Python) / `consumer_ttl` (ESPHome) to restore a fixed eviction window ([#462](https://github.com/tomquist/astrameter/issues/462)). The ESPHome `ct002` component mirrors all three changes.
