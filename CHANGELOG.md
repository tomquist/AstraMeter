...
- **Fixed** a battery on one phase repeatedly stopping and restarting its charging while a *full* battery on another phase fed excess PV to the grid (e.g. Venus E on phase C oscillating while Venus D on phase A sits at 100% SoC). The CT002/CT003 emulator recorded each battery's instructed power from only its own phase's slice of the split charge command, so a full battery involuntarily passing PV through could be mis-broadcast as *discharging* on its phase; other batteries in whole-house metering mode then throttled their charging against that phantom signal. The emulator now records the full instructed net power (matching what the battery firmware integrates from all three phase fields), so a battery told to charge is never advertised as discharging ([#447](https://github.com/tomquist/astrameter/issues/447)).
