...
- Fixed battery priority rotation getting stuck at zero targets when the grid meter went silent mid-handoff (observed with HomeWizard and HomeAssistant push-based powermeters whose WebSocket silently half-opened), leaving the grid uncompensated until manual restart. Both powermeters now send a 30 s ping/pong heartbeat and reject measurements older than 30-60 s as stale; HomeWizard additionally runs an application-layer watchdog that force-reconnects if no measurement arrives within 45 s. The meter smoother is reseeded on every probe commit/reject so post-handoff control cannot drag stale pre-probe EMA state into the new steady state, and when the powermeter raises a stale error CT002 emits a rate-limited warning log instead of flooding it and holds battery output until the upstream meter recovers.
