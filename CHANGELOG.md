...
- **Added** the same **live status dashboard to the ESPHome component**, served by the ESP32 itself at `http://<device>/`. Adding `dashboard:` under `ct002:` is the whole opt-in, and `dashboard: false` takes it back out of the firmware. The board serves a read-only version — no configuration (it is compiled into the firmware) and no battery controls — but grid power, every battery with its target and saturation, the sensor's health and the balancer's internals are all there. See [docs/dashboard.md](docs/dashboard.md#esphome-on-an-esp32).
