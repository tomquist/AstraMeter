...
- **Added** an opt-in **live status dashboard**: a single page showing grid power, every battery's target and reported power, power-source health and the balancer's internal state, plus the ability to change your configuration and control individual batteries without editing files. In the Home Assistant add-on it is on by default, opens straight from the sidebar, and configures the add-on through a guided form with a searchable picker for your grid-power sensor; with a custom `config.ini` it offers a structured editor for that file instead, and you can switch between the two. For Docker, set `DASHBOARD_ENABLED`. See [docs/dashboard.md](docs/dashboard.md) ([#577](https://github.com/tomquist/astrameter/pull/577)).
- **Fixed** edits to a custom `config.ini` being lost whenever the Home Assistant add-on restarted: the add-on read a throwaway copy of your file instead of the file itself, so any change made outside the add-on options silently disappeared at the next restart ([#577](https://github.com/tomquist/astrameter/pull/577)).
- **Added** the **ESPHome native API** as a power source (`[ESPHOMENATIVE]`): AstraMeter reads power data from an esphome device via the native API. This should be more performant than the polling mechanism of the ESPHOME component. See [docs/powermeters.md](docs/powermeters.md#esphomenative) ([#566](https://github.com/tomquist/astrameter/pull/566))
