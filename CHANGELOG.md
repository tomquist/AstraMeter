...
- **Changed** the bundled **battery simulator** to steer like a real Marstek Venus-class battery: the old `output + grid` integral rule is replaced by the gain-scheduled, accelerating self-consumption controller (ramp/acceleration curve, power-window clamp, and per-phase share-split across batteries) the real hardware runs, so simulated dynamics match hardware much more closely.
- **Fixed** relay mode (`ACTIVE_CONTROL = False`) reporting a battery count of `1` per phase regardless of how many batteries were attached. The CT002/CT003 emulator now forwards the real per-phase battery count, so several batteries on one phase each take their 1/N share of the forwarded aggregate instead of all chasing the full grid residual.
- **Added** support for the CT request's optional "participate" field (the 7th field newer batteries such as the B2500 send): a battery that sends `participate=0` is now excluded from the emulator's per-phase aggregation and from active-control distribution, so it can opt out of being counted or driven. Absent/non-zero keeps the previous behavior.
