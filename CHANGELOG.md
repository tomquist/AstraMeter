...
- Fixed battery priority rotation getting stuck at zero targets after a probe handoff, leaving the grid uncompensated until manual restart. The meter smoother is now reseeded on probe commit/reject so post-handoff control does not drag stale pre-probe EMA state into the new steady state, and the smoother dedup no longer masks fresh readings that coincidentally share a previous sample identifier.
