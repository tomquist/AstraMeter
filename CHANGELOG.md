...
- **Improved** active-control steering for **multi-battery** setups and for **sustained hunting** (e.g. a washing-machine drum pausing and restarting under a laggy meter). The oscillation damper's defaults were retuned (`OSC_DAMP_MAX` 0.8 → 0.9, `OSC_DAMP_DECAY` 0.1 → 0.07 / `osc_damp_*` on ESPHome) so a continuous hunt is damped harder while a brief post-step ring-down still passes, and ramp pacing now accounts for how many batteries share a correction so a pool reacting together doesn't over-slew its combined output into the worst overshoot peaks. Measured across the steering-evaluation suite this cuts grid oscillation and roughly halves battery power churn in the worst hunting cases, **slightly improves self-consumption** (less avoidable grid import/export — the headline metric for a zero-feed loop), and lowers overshoot and settling spread, with no change to single-battery behaviour.
