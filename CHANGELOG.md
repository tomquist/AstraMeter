...
- **Fixed** the per-battery **Manual Target** and **Auto Target** controls (and the other per-battery settings) resetting to their defaults after an AstraMeter restart. The retained command the broker redelivered on reconnect was being applied before the battery had finished starting up, so it was dropped; it's now held and re-applied once the battery is ready ([#520](https://github.com/tomquist/astrameter/discussions/520)).
