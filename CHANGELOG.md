...
- **Added** the same **live status dashboard to the ESPHome component**, served by the ESP32 itself at `http://<device>/`, showing grid power, every battery and the balancer's state. Add `dashboard:` under `ct002:` to switch it on, plus `controls: true` to steer batteries from the page; see [docs/dashboard.md](docs/dashboard.md#esphome-on-an-esp32) ([#605](https://github.com/tomquist/astrameter/pull/605)).
