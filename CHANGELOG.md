...
- **Added** a **live status dashboard**: one page showing grid power, your batteries, power-source health and — when AstraMeter is steering them — each battery's target and the balancer's internal state, plus the ability to change your configuration and steer individual batteries without editing files. On by default and read-only; in the Home Assistant add-on it opens from the sidebar, elsewhere it is at `http://<host>:52500/`. Set `DASHBOARD_ENABLED = False` to serve nothing, or `DASHBOARD_ALLOW_WRITE = True` to allow changes. See [docs/dashboard.md](docs/dashboard.md) ([#577](https://github.com/tomquist/astrameter/pull/577), [#604](https://github.com/tomquist/astrameter/pull/604)).

- **Added** the same **live status dashboard to the ESPHome component**, served by the ESP32 itself at `http://<device>/` with no configuration at all, showing grid power, every battery and the balancer's state. Add `controls: true` under `dashboard:` to steer batteries from the page, or `dashboard: false` to leave it out of the firmware; see [docs/dashboard.md](docs/dashboard.md#esphome-on-an-esp32) ([#605](https://github.com/tomquist/astrameter/pull/605)).
