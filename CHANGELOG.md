...
- Fixed CT002 battery polling oscillations and post-probe lockup for mixed V2/V3 Marstek fleets: `DEDUPE_TIME_WINDOW` now accepts fractional seconds (so V3 batteries polling at ~0.45 s no longer crash the config loader); the efficiency saturation tracker is time-weighted so V2 (~3 s) and V3 (~0.45 s) batteries converge to the same saturation score under identical conditions; and the post-probe rotation fade no longer pollutes the saturation EMA of the outgoing consumer, which previously left the balancer unable to steer any battery and the grid could drift uncompensated for over an hour.
