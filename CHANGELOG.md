...
- **Improved** active-control steering for **multi-battery** setups and for **sustained hunting** (e.g. a washing-machine drum pausing and restarting under a laggy meter). Ramp pacing is now shared across the pool: several batteries reacting to the same grid reading no longer slew their *combined* output proportionally to how many there are, so a two- or three-battery pool overshoots about as little as a single one after a load step instead of several times more. The oscillation damper's defaults were also retuned (`OSC_DAMP_MAX` 0.8 → 0.9, `OSC_DAMP_DECAY` 0.1 → 0.07 / `osc_damp_*` on ESPHome) so a continuous hunt is damped harder while a brief post-step ring-down still passes — together these roughly halve battery power churn and grid oscillation in the worst hunting cases. Measured across the steering-evaluation suite, overshoot, hunting frequency, settling spread and control effort all drop, with no change to single-battery behaviour.
