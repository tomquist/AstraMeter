...
- **Added** a per-powermeter **Online** diagnostic sensor in MQTT Insights: each configured power source gets an "AstraMeter Powermeter `<section>`" device whose connectivity sensor flips off when the source stops delivering fresh, usable readings (a stalled or disconnected stream, or a failing poll), so you can alert when AstraMeter is silently holding battery output on a stale value. A phase that merely stops changing stays online; tune or disable the publish cadence with `POWERMETER_HEALTH_INTERVAL` ([#427](https://github.com/tomquist/astrameter/issues/427)).
