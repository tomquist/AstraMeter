...
- **Fixed** three active-control regulation flaws found while chasing [#469](https://github.com/tomquist/astrameter/issues/469), each cutting hunting, overshoot, or reaction time across the simulated household scenario suite: (1) efficiency optimization (`MIN_EFFICIENT_POWER`) no longer toggles a battery between active and parked on every meter-noise tick when demand sits near a multiple of the floor — the active-slot count now uses the same 20% hysteresis as entering/exiting limiting; (2) ramp pacing no longer deadlocks a step response at ~10 W/poll (the battery firmware's minimum step on a constant reading sat below pacing's "is it tracking" threshold, leaving 2 kW load steps crawling for minutes — settle times drop 40-70% in most scenarios and standing import leaks shrink up to ~70%); and (3) pacing is now time-based (W per second instead of W per poll), so a fast-polling battery (e.g. Venus E at ~0.45 s) no longer slews more than twice as hard as a 1 s one against the same meter — worst-case wrong-direction swings in mixed-cadence pools drop by half or more. Deprioritized batteries also wind down to zero through the same pacing instead of dumping their whole output in one poll.
