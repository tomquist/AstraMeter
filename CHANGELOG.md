...
- **Fixed** efficiency optimization (`MIN_EFFICIENT_POWER`) hunting indefinitely when demand sits near a multiple of the configured floor with three or more batteries (e.g. a ~300 W base load with a 150 W floor): the number of active slots now uses the same 20% hysteresis as entering/exiting limiting, so meter noise no longer toggles a battery between active and parked on every poll. In the mixed Venus + B2500 evaluation scenario this halves settle time and battery wear and brings efficiency mode in line with plain fair-share ([#469](https://github.com/tomquist/astrameter/issues/469)).
