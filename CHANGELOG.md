...
- **Added** adaptive feedback-lag compensation to active control: the grid meter the controller steers against is always a little stale (a P1 dongle or Home Assistant push sensor measures, then takes a second or two to arrive, and holds the value between refreshes), and acting on that stale reading is the main cause of overshoot and hunting. AstraMeter now advances the stale reading to the present using the batteries' own fresh reported outputs, and **learns the effective lag online per install** — so you no longer have to find the right throttle interval for your meter. Across the simulated scenario matrix this cuts worst-case overshoot ~40%, hunting ~50% and battery churn ~25% while settling no slower, and it lets active control recover on its own when a push meter goes briefly silent mid-handoff. It is on by default and self-limits to ~zero on a fast meter; tune the upper bound or disable it with the new `PREDICT_LAG_S` option (`predict_lag_s` on ESPHome).
