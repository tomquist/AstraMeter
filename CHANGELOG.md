...
- **Added** experimental ESPHome external component `ct002` that ports the CT002/CT003 UDP emulator, balancer, and cross-phase filter pipeline (Hampel / smoothing / deadband / PID) to run directly on an ESP32, reading grid power from any ESPHome-compatible sensor (Modbus, M-Bus, Tasmota, etc.). Two optional sub-blocks under the same `ct002:` key: `mqtt_insights:` publishes Home Assistant Device Discovery (per-consumer + device-level entities, including manual-target/active/auto-target controls and a force-rotation button) and answers Marstek-app polls on the local broker so the emulator shows up in the app without hame-relay round-tripping; `marstek_registration:` runs the cloud device-registration flow from `[MARSTEK]` (token → list → maybe-add → confirm) on first boot, persists the resulting MAC via ESPPreferences so subsequent boots skip the cloud round-trip, and wires the MAC back into `ct002.ct_mac` so UDP responses and Marstek MQTT topics line up with the cloud-side record. Configure via `external_components: { source: github://tomquist/astrameter@main, components: [ct002] }` and the nested schema; see `esphome.example.yaml`. Also **fixed** 1-phase active balancing in the Python emulator: `_compute_smooth_target` previously bailed out for any non-3-tuple input and returned the raw grid reading unchanged, so single-phase users got passive forwarding instead of balanced targets despite single-phase being a documented supported configuration. The guard now only rejects empty values; the balancer collapses values to a scalar total internally, so any phase count works.
