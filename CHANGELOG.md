...
- **Added** a per-battery **Efficiency Window Weight** control (Home Assistant entity, Python and ESPHome) that biases the balancer's efficiency rotation — when household demand is low and only some batteries run to stay efficient, this sets how much of that rotation window each battery takes: `100 %` for full participation, `0 %` to skip a battery for efficiency (parked while limiting, but still used when all batteries are needed), or anything in between for proportionally less active time and wear. Separate from **Distribution Weight**, which biases the split among active batteries.
