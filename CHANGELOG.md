...
- **Fixed** the Home Assistant add-on losing edits saved from the built-in config editor when a custom config file (`custom_config`) is used — the add-on worked on a throwaway copy, so every saved change was silently reverted at the next restart. It now edits your file directly ([#578](https://github.com/tomquist/astrameter/pull/578)).
