...
- **Fixed** `DEVICE_TYPE = shellypro3em_new` (and `shellypro3em_old`) not being recognized by the battery: when selected on their own they were assigned a generic `device-N` id instead of a Shelly-style `shellypro3em-…` id, so the battery never matched the CT. Both variants now use the proper Shelly id prefix, matching the combined `shellypro3em` mode ([#389](https://github.com/tomquist/astrameter/issues/389)).
