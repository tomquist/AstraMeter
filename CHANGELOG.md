...
- **Changed** active control to act on an **adaptive, self-learning estimate of the grid** instead of the raw meter reading, which cuts grid import *and* export and removes the need to tune the controller for your specific powermeter. Batteries report their own output faster than most grid meters refresh, so the balancer now credits each battery's just-delivered output between meter updates — it no longer keeps re-issuing a correction that is already on its way, the main cause of overshoot and back-and-forth hunting — and it learns online how much to trust each new meter reading (more during a real load/solar step, less when the loop starts to hunt). The same setting then works whether your meter refreshes once a second or arrives several seconds late (e.g. a Home Assistant push sensor). Across the simulated household scenarios this reduces avoidable grid import/export by ~5%, overshoot by ~40%, and battery power churn by ~28%. On by default; tune the initial trust or disable it with the new `GRID_PREDICT_TRUST` option (`grid_predict_trust` on ESPHome).
